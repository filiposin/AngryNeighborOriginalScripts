...
public class CharacterInventory : MonoBehaviour
{
	...
	public void RemoveItemByCollectorSync(ItemCollector itemcollector, int num)
	{
		...
		if ((bool)networkViewer)
		{
			networkViewer.RPC("removeItemSync", RPCMode.All, num2, num);
		}
	}
	...
	public void DropItem(ItemData itemdata, int num, int numtag)
	{
		...
		if (RemoveItem(itemdata, num))
		{
			...
			{
				gameObject = (GameObject)Network.Instantiate(itemdata.gameObject, base.transform.position, itemdata.gameObject.transform.rotation, 2);
	...
	public void DropItemByCollector(ItemCollector item, int num)
	{
		...
		if (RemoveItemByCollector(item, num))
		{
			...
			if (Network.isServer || Network.isClient)
			{
				gameObject = (GameObject)Network.Instantiate(item.Item.gameObject, base.transform.position, item.Item.gameObject.transform.rotation, 2);
				ItemData component = gameObject.GetComponent<ItemData>();
	...
	private void Update()
	{
		...
		if (IsReady && character != null && (Network.isServer || Network.isClient) && character.IsMine && (bool)networkViewer)
		{
