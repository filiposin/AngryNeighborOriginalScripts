...
public class GUILevelPresetsLoader : MonoBehaviour
{
	...
	{
		if (Canvas == null || LevelPrefab == null || !(UnitZ.sceneManager != null))
		{
			return;
		}
		ClearCanvas();
		for (int i = 0; i < UnitZ.sceneManager.LevelPresets.Length; i++)
		{
			...
			if ((bool)component)
			{
				component.Level = UnitZ.sceneManager.LevelPresets[i];
			}
		...
		component3.sizeDelta = new Vector2(component3.sizeDelta.x, (LevelPrefab.sizeDelta.y + 5f) * (float)UnitZ.sceneManager.LevelPresets.Length);
	}
}
