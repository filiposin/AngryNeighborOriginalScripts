...
public class GUICharacterPresetLoader : MonoBehaviour
{
	...
	public void Draw()
	{
		if (Canvas == null || CharacterPresetPrefab == null || View == null || !(UnitZ.sceneManager != null))
		{
			return;
		}
		ClearCanvas();
		...
		float num2 = (CharacterPresetPrefab.sizeDelta.x + 5f) * (float)UnitZ.characterManager.CharacterPresets.Length;
		...
		component.sizeDelta = new Vector2(num + num + (CharacterPresetPrefab.sizeDelta.x + 5f) * (float)UnitZ.characterManager.CharacterPresets.Length, component.sizeDelta.y);
	}
}
