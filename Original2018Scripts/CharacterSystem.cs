...
public class CharacterSystem : DamageManager
{
	...
	{
		if ((bool)networkViewer && (IsMine || (ID == string.Empty && networkViewer.isMine)) && (Network.isClient || Network.isServer))
		{
			networkViewer.RPC("UpdateTransform", RPCMode.Others, base.transform.position, base.transform.rotation, Motor.OjectVelocity);
		}
	}
	...
	{
		if ((bool)networkViewer && (Network.isClient || Network.isServer))
		{
	...
	public void DoDamage(Vector3 origin, Vector3[] direction, int damage, float distance, int penetrate, string id, string team)
	{
		if ((bool)networkViewer && (Network.isServer || Network.isClient))
		{
			...
			if ((bool)rayActive && rayActive.ShootRayTest(origin, direction, damage, distance, penetrate, id, team))
			{
				PlayDamageSound();
			}
		}
	...
	public void DoDamage()
	{
		if ((bool)networkViewer && (Network.isServer || Network.isClient))
		{
			networkViewer.RPC("doDamage", RPCMode.All, base.transform.position + DamageOffset, base.transform.forward, Damage, DamageLength, Penetrate, ID, Team);
		}
		else
		{
