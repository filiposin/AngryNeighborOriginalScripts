	...
	private void Start()
	{
		if ((bool)Item)
		{
			if (Network.isServer || Network.isClient)
			{
				if (Network.isServer)
				{
					if (ItemUID == string.Empty)
					{
						GenItemUID();
					}
					GameObject gameObject = (GameObject)Network.Instantiate(Item, base.transform.position, base.transform.rotation, Group);
			...
			{
				GameObject gameObject2 = UnityEngine.Object.Instantiate(Item, base.transform.position, base.transform.rotation);
