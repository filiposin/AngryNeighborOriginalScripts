...
public class followCam : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		{
			float y3 = target.eulerAngles.y;
			float b3 = target.position.y + height;
			float y4 = base.transform.eulerAngles.y;
			float y5 = base.transform.position.y;
			y4 = Mathf.LerpAngle(y4, y3, rotationDamping * Time.deltaTime);
			y5 = Mathf.Lerp(y5, b3, heightDamping * Time.deltaTime);
			Quaternion quaternion2 = Quaternion.Euler(0f, y4, 0f);
			movement = new Vector3(target.position.x, y5, target.position.z - distance);
			base.transform.position = target.position;
			base.transform.position -= quaternion2 * Vector3.forward * distance;
			base.transform.position = new Vector3(target.position.x, y5, target.position.z - distance);
