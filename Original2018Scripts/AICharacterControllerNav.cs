...
public class AICharacterControllerNav : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if ((bool)NavObject && (Network.isServer || (!Network.isClient && !Network.isServer)))
		{
			GameObject gameObject = Object.Instantiate(NavObject.gameObject, base.transform.position, base.transform.rotation);
			nav = gameObject.GetComponent<NavAgent>();
			nav.Owner = base.gameObject;
		}
	}
	...
	private void Update()
	{
		...
		if (!flag)
		{
			character.networkViewer.RPC("MoveToPosition", RPCMode.All, vector);
		}
		else
		{
